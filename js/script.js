window.onload = function () {
  ...
  var i,
    bubblesNumber = w * h > 750000 ? 200 : 150,
    objects = [],
  ...
  window.addEventListener("mousemove", function (e) {
    for (var i = 0; i < world.objects.length; i++) {
      if (
        ...
        e.clientX > world.objects[i].x - world.objects[i].radius &&
        e.clientX < world.objects[i].x + world.objects[i].radius &&
        e.clientY < world.objects[i].y + world.objects[i].radius &&
        e.clientY > world.objects[i].y - world.objects[i].radius
      ) {
  ...
  window.addEventListener("touchmove", function (e) {
    for (var i = 0; i < world.objects.length; i++) {
      if (
        world.objects[i] instanceof Bubble &&
        e.touches[0].clientX > world.objects[i].x - world.objects[i].radius &&
        e.touches[0].clientX < world.objects[i].x + world.objects[i].radius &&
        e.touches[0].clientY < world.objects[i].y + world.objects[i].radius &&
        e.touches[0].clientY > world.objects[i].y - world.objects[i].radius
      ) {
        world.objects[i].pop(world);
      }
